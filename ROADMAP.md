...
- [ ] **Hermetic Toolchain Downloader**: Fetch declared Zig/Go/Node/CMake toolchains into a versioned local store with checksum pinning. *(URL tables and traversal-hardened path logic already exist in `fish-core/src/toolchain_downloader.rs`; network fetch and verification remain.)*
- [ ] **Toolchain Lock File**: Commit a `fish.lock` capturing exact toolchain versions per backend for reproducible CI.
...
- [ ] **Environment Drift Detector**: Diff the effective toolchain/env snapshot against the last successful build and warn on drift.
