...
pub fn analyze_compiler_error(
    ...
    if toolchain == "rust" || lower.contains("rustc") || lower.contains("cargo") {
        if error_text.contains("cannot find")
            || error_text.contains("E0425")
            || error_text.contains("E0433")
        {
            return DiagnosticReport {
                category: "UNDEFINED_SYMBOL_ERROR",
                root_cause:
                    "A type, function, or module is referenced but not imported or declared in scope."
                        .to_string(),
                suggested_fixes: vec![
            ...
            return DiagnosticReport {
                ...
                suggested_fixes: vec![
                    "Inspect function parameter and return types for conversion methods (e.g. `.into()`, `.as_ref()`)."
                        .to_string(),
                ],
            };
        } else if error_text.contains("borrow")
            || error_text.contains("E0502")
            || error_text.contains("E0382")
        {
            return DiagnosticReport {
                category: "BORROW_CHECKER_ERROR",
                root_cause:
                    "Value used after move or simultaneous mutable and immutable borrows detected."
                        .to_string(),
                suggested_fixes: vec![
    ...
    if (toolchain == "ts"
        ...
        && (lower.contains("cannot find module") || lower.contains("ts2307"))
    {
        return DiagnosticReport {
            category: "MISSING_MODULE_ERROR",
            root_cause:
                "Imported package or module is not installed in node_modules or path alias is missing."
                    .to_string(),
            suggested_fixes: vec![
    ...
    if (toolchain == "go" || lower.contains("go build")) && lower.contains("undefined:") {
        return DiagnosticReport {
            category: "GO_UNDEFINED_IDENTIFIER",
            root_cause: "Identifier is not declared in the current package or imported packages."
                .to_string(),
            suggested_fixes: vec![
                "Run `go mod tidy` to update dependencies.".to_string(),
                "Verify exported symbol casing (first letter uppercase for public identifiers)."
                    .to_string(),
            ],
        };
    }
    ...
    if (toolchain == "cc" || toolchain == "cpp" || lower.contains("clang") || lower.contains("gcc"))
        && (lower.contains("fatal error:") || lower.contains("no such file or directory"))
    {
        return DiagnosticReport {
            category: "MISSING_HEADER_ERROR",
            root_cause: "Include header file could not be resolved in compiler search paths."
                .to_string(),
            suggested_fixes: vec![
                "Add `-I<include_dir>` flag or `target_include_directories` in CMakeLists.txt."
                    .to_string(),
            ],
        };
    }
    ...
    DiagnosticReport {
        ...
        suggested_fixes: vec![
            ...
            "Run with verbose output `fish build -v --explain` to diagnose rebuild triggers."
                .to_string(),
        ],
    }
}
