...
fn query_gemini_healer(api_key: &str, raw_errors: &[String]) -> Option<String> {
    ...
    let prompt = format!(
        "You are an elite Rust systems engineer and build diagnostic AI for Fish build tool. Analyze these build errors and provide a concise, high-impact root cause diagnosis and direct code solution:\n\n```\n{}\n```",
        joined_errors
    );
