...
fn get_system_stats_json(_root: &Path) -> String {
    ...
    let cache_dir = env::var("FISH_CACHE_DIR")
        .map(PathBuf::from)
...
fn dirs_or_temp_cache() -> PathBuf {
    if let Ok(home) = env::var("USERPROFILE").or_else(|_| env::var("HOME")) {
        ...
        env::temp_dir().join("fish").join("cache")
    }
}
...
<head>
    ...
    <title>Fish Telemetry & Interactive DAG Visualizer</title>
    <style>
        :root {
            --bg-base: #090d16;
            --bg-card: #111827;
