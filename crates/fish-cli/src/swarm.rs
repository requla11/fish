...
impl SwarmCache {
    ...
    pub fn broadcast_presence(&self, service_port: u16) -> std::io::Result<()> {
        ...
        if let Some(ref socket) = self.socket {
            let message = format!("FISH_SWARM_ANNOUNCE:{}:{}", self.peer_id, service_port);
        ...
        if let Some(ref socket) = self.socket {
            ...
            while let Ok((amt, src)) = socket.recv_from(&mut buf) {
                if let Ok(msg) = std::str::from_utf8(&buf[..amt]) {
                    if let Some(payload) = msg.strip_prefix("FISH_SWARM_ANNOUNCE:") {
                    ...
                    } else if let Some(payload) = msg.strip_prefix("FISH_SWARM_COMPUTE:") {
                        let parts: Vec<&str> = payload.split(':').collect();
                        if parts.len() == 3 {
                            let peer_id = parts[0].to_string();
                            if peer_id != self.peer_id
