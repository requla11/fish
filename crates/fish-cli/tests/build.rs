...
fn version_flag_prints_name_and_version() {
    let output = run(fish().arg("--version"));
    assert!(output.status.success());
    assert!(stdout(&output).contains(&format!("fish {}", env!("CARGO_PKG_VERSION"))));
}
...
fn build_rebuilds_from_the_fingerprint_cache() {
    let dir = workspace_fixture();
    let cache_dir = dir.path().join(".fish-cache");
    let first = run(fish()
        .arg("build")
        .arg("--cache-dir")
        .arg(&cache_dir)
...
fn build_discovers_project_upward_from_nested_directory() {
    ...
    let output = run(fish().arg("build").current_dir(dir.path().join("src")));
    assert!(output.status.success(), "stderr: {}", stderr(&output));
...
fn check_type_checks_the_workspace() {
    let dir = workspace_fixture();
    let output = run(fish().arg("check").current_dir(dir.path()));
    assert!(output.status.success(), "stderr: {}", stderr(&output));
...
fn test_runs_workspace_tests_and_reports_success() {
    ...
    let output = run(fish().arg("test").current_dir(dir.path()));
    assert!(output.status.success(), "stderr: {}", stderr(&output));
    let text = stdout(&output);
...
fn clean_removes_build_artifacts() {
    let dir = workspace_fixture();
    let build = run(fish().arg("build").current_dir(dir.path()));
    ...
    let output = run(fish().arg("clean").current_dir(dir.path()));
    assert!(output.status.success(), "stderr: {}", stderr(&output));
...
fn build_rejects_a_file_path() {
    ...
    let output = run(fish().arg("build").arg(dir.path().join("src/lib.rs")));
...
fn graph_tree_prints_workspace_graph() {
    let dir = workspace_fixture();
    let output = run(fish().arg("graph").current_dir(dir.path()));
    assert!(output.status.success(), "stderr: {}", stderr(&output));
...
#[test]
fn fish_toml_sets_worker_count_and_disable_flag_wins() {
    let dir = workspace_fixture();
    fs::write(
...
fn build_with_sandbox_flag_succeeds() {
    let dir = workspace_fixture();
    let output = run(fish().arg("build").arg("--sandbox").current_dir(dir.path()));
    assert!(output.status.success(), "stderr: {}", stderr(&output));
    let text = stdout(&output);
    assert!(text.contains("✓ network"));
    assert!(text.contains("✓ core"));
...
fn watch_mode_runs_initial_build_and_watches() {
    let dir = workspace_fixture();
    let output = run(fish().arg("watch").arg("--once").current_dir(dir.path()));
...
fn build_python_project_succeeds() {
    ...
    fs::write(dir.path().join("fish.py.json"), fish_py).expect("write fish.py.json");
    fs::create_dir(dir.path().join("src")).expect("mkdir src");
    fs::write(
...
fn build_with_tui_flag_succeeds() {
    let dir = workspace_fixture();
    let output = run(fish().arg("build").arg("--tui").current_dir(dir.path()));
    assert!(output.status.success(), "stderr: {}", stderr(&output));
}
...
fn build_with_remote_cache_flags_succeeds() {
    ...
    let server = fish_remote_cache::RemoteCacheServer::new(
        &cache_addr,
...
fn build_with_remote_workers_flags_succeeds() {
    ...
    let server = fish_worker::WorkerServer::with_options(
        &worker_addr,
...
fn doctor_reports_tools_and_cache() {
    ...
    let output = run(fish()
        .arg("doctor")
        .env("FISH_CACHE_DIR", dir.path().join("cache")));
    assert!(output.status.success(), "stderr: {}", stderr(&output));
    let text = stdout(&output);
    assert!(
