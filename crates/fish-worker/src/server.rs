...
use fish_remote_cache::artifact::unpack_artifacts;
...
impl WorkerServer {
    pub fn new(addr: impl Into<String>, auth_token: Option<String>) -> Self {
        Self::with_options(addr, auth_token, "fish-worker-node", 8)
    }
            ...
            let response = match outcome {
                Ok(out) => RemoteTaskResponse {
                    ...
                    exit_code: out.exit_code.or(
                        if out.status == fish_executor::TaskStatus::Failed {
                            Some(1)
                        } else {
                            Some(0)
                        },
