...
impl TsProjectConfig {
    pub fn from_forge_file(path: &Path) -> Result<Self, String> {
    ...
    pub fn discover_or_default(root: &Path) -> Result<Self, String> {
        ...
        if forge_ts_path.exists() {
            return Self::from_forge_file(&forge_ts_path);
        }

        let pkg_path = root.join("package.json");
        if !pkg_path.exists() {
            return Err("neither forge.ts.json nor package.json found".to_string());
        }
