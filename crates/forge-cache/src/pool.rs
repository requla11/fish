...
impl BufferPool {
    pub fn new() -> Self {
        Self {
            pools: SIZE_TIERS.iter().map(|_| Mutex::new(VecDeque::new())).collect(),
...
impl ScopedBuffer {
    ...
    /// Convert into inner buffer (takes ownership, won't return to pool)
    pub fn into_inner(mut self) -> Vec<u8> {
        // Prevent returning to pool by clearing the pool reference
