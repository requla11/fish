...
impl CasBackend for LocalCasBackend {
    async fn store(&self, artifact: &Artifact) -> Result<()> {
        ...
        let (data_to_store, compressed_size) = if self.compression != crate::compression::CompressionAlgorithm::None {
    ...
    async fn stats(&self) -> Result<CasStats> {
        ...
        for hash in &hashes {
            if let Ok(artifact) = self.retrieve(hash).await {
                ...
                if let Some(comp_size) = artifact.metadata.compressed_size {
...
impl RemoteCasBackend for RemoteCasBackendImpl {
    async fn upload(&self, _artifact: &Artifact) -> Result<()> {
        ...
        Err(CasError::BackendError("Remote upload not yet implemented".to_string()))
    }
    
    async fn download(&self, _hash: &ArtifactHash) -> Result<Artifact> {
        // TODO: Implement actual download
        Err(CasError::BackendError("Remote download not yet implemented".to_string()))
    }
...
impl CasBackend for HybridCasBackend {
    ...
    async fn exists(&self, hash: &ArtifactHash) -> Result<bool> {
        self.local.exists(hash).await.or_else(|_| self.remote.exists(hash).await)
    }
