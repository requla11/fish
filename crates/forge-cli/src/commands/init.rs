...
pub fn generate_forge_yaml(languages: &[DetectedLanguage]) -> String {
...
pub fn run_init(path: Option<PathBuf>, force: bool) -> ExitCode {
    ...
    let forge_file = target_dir.join("forge.yaml");

    if forge_file.exists() && !force {
        eprintln!(
    ...
    println!("🦀 Initializing Forge in {}", target_dir.display());
    ...
    let config_content = generate_forge_yaml(&detected);
    if let Err(e) = std::fs::write(&forge_file, config_content) {
        eprintln!("error: failed to write forge.yaml: {e}");
        return ExitCode::FAILURE;
    }

    println!("  [ok] Successfully generated {}", forge_file.display());
    println!("\nYou can now run:");
