...
fn main() {
    ...
    println!("Hello, {}! Powered by Forge.", recipient);
}
"#;
    fs::write(dir.join("src/main.rs"), main_rs)?;

    let forge_yaml = r#"version: "1"
...
"#;
    fs::write(dir.join("forge.yaml"), forge_yaml)?;
...
tasks:
  ...
  test:
    depends_on:
      ...
      - worker-test
"#;
    fs::write(dir.join("forge.yaml"), root_forge)?;
