...
mod tests {
    ...
    fn test_fs_watcher_daemon_dirty_tracking() {
        ...
        watcher.mark_dirty("crates/forge-core/src/lib.rs");
        watcher.mark_dirty("crates/forge-cli/src/main.rs");

        assert_eq!(watcher.dirty_count(), 2);
        assert!(watcher.is_dirty("crates/forge-core/src/lib.rs"));

        watcher.mark_clean("crates/forge-core/src/lib.rs");
        assert_eq!(watcher.dirty_count(), 1);
        assert!(!watcher.is_dirty("crates/forge-core/src/lib.rs"));

        let remaining = watcher.take_invalidated_targets();
        assert_eq!(remaining.len(), 1);
        assert_eq!(remaining[0], "crates/forge-cli/src/main.rs");
        assert_eq!(watcher.dirty_count(), 0);
    }
