...
- **ビットレベルの読み取り検証と自己修復 (Self-Healing)**: 読み取りごとに BLAKE3 ハッシュを自動照合。ビット破損や不正オブジェクトを即座に破棄し、クリーンな再ビルドを強制。
- **ワークスペース成果物の完全性ガード**: キャッシュヒット時にワークスペース成果物の BLAKE3 チェックサムを精査し、手動改ざんや切り詰められた成果物による誤ヒットを防止。
- **高耐久アトミック書き込み**: ファイルのリネーム前に `sync_all()` でディスクに同期し、突然の電源断や強制終了による 0 バイト破損ファイルを防止。
- **キャッシュ監査 & 修復 CLI (`fish cache verify [--repair]`)**: 指紋レコードとオブジェクトをディープスキャンし、孤立または破損したデータを自動クリーンアップ。
