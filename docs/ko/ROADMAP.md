...
- [x] **Web Team Analytics Dashboard**: 집계된 빌드 속도 향상, 캐시 적중 효율성, 팀 생산성 지표. *(`fish-dashboard`의 JSON API를 갖춘 실제 HTTP 서버: `/api/builds` GET/POST, `/api/traces`, `/api/team-stats` (중간 지속 시간, 캐시 적중률, 성공/실패 횟수), `/api/builds/{id}/flamegraph`. `PersistentMetricsStore`는 재시작 후에도 지표가 유지되도록 JSONL 영속성으로 대시보드를 지원합니다; `ApiState`는 시작 시 데이터를 다시 로드합니다.)*
