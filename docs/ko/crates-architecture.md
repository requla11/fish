...
Fish is composed of 27 modular Rust crates structured into clear architectural tiers.
...
4. **Toolchains & Backends (11 Crates + contract)**:
   - `fish-backend-api`: The shared `EcosystemBackend` contract (trait, BuildMode, Ecosystem).
...
5. **Security & Diagnostics Tier**:
   - `fish-security`, `fish-analytics`, `fish-incremental`, `fish-installer`.
6. **Application Tier**:
   - `fish-cli`: Unified command-line interface, TUI dashboard, LSP server, backend registry.
