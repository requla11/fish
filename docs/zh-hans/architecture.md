...
- **读取期逐比特校验与自愈 (Self-Healing)**：每次读取时自动核对 BLAKE3 散列值；损毁或比特衰减 (bit-rot) 的文件将被立即剔除并强制纯净重构。
- **工作区产物一致性哨兵**：缓存命中时深入核查工作区产物的 BLAKE3 校验和，杜绝手工篡改或非完整产物引发的伪命中。
- **硬件级持久化原子写入**：重命名文件前调用 `sync_all()` 强制刷盘，防止突发断电或进程强杀残留 0 字节损坏文件。
- **缓存审计与修复工具 (`fish cache verify [--repair]`)**：端到端深度扫描指纹记录与对象存储，自动清理孤立或损坏数据。
