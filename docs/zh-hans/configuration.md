...
本指南介绍如何使用 `fish.toml` 配置 Fish 工作区。

---

## 配置文件概览

Fish 会读取工作区根目录下的 `fish.toml` 配置文件。如果未提供 `fish.toml`，Fish 将自动应用一套合理的默认值。
...
| `sandbox` | 布尔值 | `false` | 在沙箱隔离环境中执行构建任务。 |
...
| 配置键 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `cache_url` | 字符串 | None | 远程缓存服务器地址 (HTTP)。 |
| `token` | 字符串 | None | 远程访问所需的 Bearer 认证 Token。 |
| `workers` | 字符串列表 | `[]` | 远程执行集群 Worker 节点地址列表（例如 `["worker1:9000", "worker2:9000"]`）。 |
| `send_source` | 布尔值 | `false` | 压缩并传输源码快照至无共享存储的远程 Worker 节点。 |
