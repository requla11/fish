...
本文档汇总了 Fish 的常见问题解答、迁移建议以及故障排查方案。
...
1. 进入您的项目根目录。
2. 运行 `fish build` 让 Fish 自动发现整个工作区。
3. （可选）在根目录下创建 `fish.toml` 以自定义流水线依赖关系和缓存路径。

### 3. Fish 的 CAS 内容寻址缓存是如何工作的？
Fish 会对输入文件、工具链版本和环境变量计算 Blake3 唯一指纹。当任务生成输出产物时，产物会经由 Zstandard 压缩并存入内容寻址存储目录 (`~/.fish/cache`)。如果输入未发生变化，Fish 将直接通过写时复制 (CoW) 或硬链接极速还原产物，完全跳过编译器的调用。
...
```
Fish 的资源管控器将在内存占用超过阈值时自动节流，减少并发任务数。
