...
本文档全面介绍了 Fish 的系統架構、核心引擎模块以及执行流水线。

---

## 系统概覽

Fish 是一个专为多语言 Monorepo 及分布式开发环境设计的高性能、缓存优先构建编排系统。Fish 并不取代各语言的原生编译器，而是作为统一的智能协调层，负责管理依赖 DAG 图、内容寻址存储（CAS）、沙箱隔离以及并发工作窃取（work-stealing）調度。
...
### 1. 工作区发现 (`fish-core`)
- **清單解析与发现**：扫描并解析 `Cargo.toml`, `package.json`, `go.mod`, `pyproject.toml`, `CMakeLists.txt`, `pom.xml`, `*.csproj`, `Package.swift`, `pubspec.yaml`, `build.zig`, `Dockerfile`。
...
### 2. 构建依赖图 (`fish-graph`)
- **拓撲任务图**：构建任务的有向无环图 (DAG) 并进行循环依赖检测。
...
- **进程調度与监控**：支持超时控制与输出流捕获的非阻塞异步任务执行。
- **快速数据克隆 (Fast Extents Cloning)**：利用写时复制 (CoW) 与硬链接极速实体化产物，零磁盘 I/O 复制开销 (`KernelCowCloner`)。
- **链接器智能調度**：自动检测并适配 `mold`, `lld`, `lld-link` 与 `msvc` 的链接参数 (`LinkerDispatcher`)。
- **编译器回應參數文件**：在命令行参数超出操作系统限制时自动生成 `@fish_args.rsp` 回應參數文件。

### 4. 調度器与系统资源管控 (`fish-scheduler`)
- **并行工作窃取 (Parallel Work-Stealing)**：在所有可用硬件核心上实现无锁高效調度。
...
- **分層复合缓存架构**：支持 L1 本地内存/磁盘缓存与 L2 远程 S3/HTTP 缓存的无缝集成。
- **讀取期逐位元校驗與自愈 (Self-Healing)**：每次讀取時自動核對 BLAKE3 雜湊值；損毀或位元衰減 (bit-rot) 的檔案將被立即剔除並強制純淨重構。
- **工作區產物一致性哨兵**：快取命中時深入核查工作區產物的 BLAKE3 校驗和，杜絕手工篡改或非完整產物引發的偽命中。
- **硬體級持久化原子寫入**：重新命名檔案前呼叫 `sync_all()` 強制刷盤，防止突發斷電或處理程序強殺殘留 0 位元組損壞檔案。
- **快取稽核與修復工具 (`fish cache verify [--repair]`)**：端到端深度掃描指紋記錄與物件儲存，自動清理孤立或損壞資料。


### 6. 用户界面与遥测 (`fish-cli`)
- **命令行界面**：提供针对 build, test, check, graph, doctor, query, affected 及 daemon 管理的完整子命令。
- **交互式 SVG DAG 可视化儀表板**：支持平移、缩放、节点聚焦、关键路径高亮等交互操作的实时 Web 画布。
- **内置 5 语言国际化**：原生内置英语、越南语、簡體中文、繁體中文和日语。
...
- **产物密碼學签名 (`fish-signing`)**：采用 Ed25519 算法生成与验证数字签名。
- **SBOM 软件物料清單导出**：支持 SPDX 与 CycloneDX 标准格式。
