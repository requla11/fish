...
inference**: four projects in three toolchains are wired together by nothing
...
| `web-frontend` | `import { EVENT_TOPICS } from "../../py-worker/contracts/topics.json"` | Type-check failure if contract breaks |
...
and web without anyone declaring it. You'll see it in the build output:

```text
Linked N cross-project dependencies (disable with --no-infer-deps)
```
...
fish graph --format dot > demo.dot
